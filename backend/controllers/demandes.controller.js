...
// "Activité hors-équipe" — le responsable connecté consulte celles qui concernent
// les équipes qu'il gère (sous-équipes UP et/ou équipes hors UP), en lecture seule
// comme la vue admin.
export async function listDemandesResponsable(req, res) {
  try {
    const demandes = await getDemandesByResponsable(req.user.id)
    res.json(demandes)
  } catch (err) {
    console.error(err)
    res.status(500).json({ message: 'Erreur serveur' })
