...
// Activités hors-équipe concernant les équipes gérées par le responsable connecté —
// utilisées par la page "Activité hors-équipe" côté responsable (même vue en lecture
// seule que côté admin, mais limitée à ses propres équipes). Une activité peut être
// liée soit via la colonne historique id_sous_equipe/id_up (une seule équipe choisie),
// soit via demande_equipe_notification (plusieurs équipes choisies) — on couvre les deux.
export async function getDemandesByResponsable(idResponsable) {
  const [rows] = await pool.query(
    `SELECT DISTINCT d.*, c.nom AS collaborateur_nom, se.nom AS sous_equipe_nom, up.nom_up AS up_nom,
       ${EQUIPES_NOMS_SUBQUERY} AS equipes_noms
     FROM demande_hors_equipe d
     JOIN collaborateur c ON c.id_collaborateur = d.id_collaborateur
     LEFT JOIN sous_equipe se ON se.id_sous_equipe = d.id_sous_equipe
     LEFT JOIN equipe_hors_up up ON up.id_up = d.id_up
     WHERE
       (d.id_sous_equipe IS NOT NULL AND d.id_sous_equipe IN (SELECT id_sous_equipe FROM sous_equipe WHERE id_responsable = ?))
       OR (d.id_up IS NOT NULL AND d.id_up IN (SELECT id_up FROM equipe_hors_up WHERE id_responsable = ?))
       OR d.id_demande IN (
         SELECT den.id_demande FROM demande_equipe_notification den
         WHERE (den.type_equipe = 'sous_equipe' AND den.id_equipe IN (SELECT id_sous_equipe FROM sous_equipe WHERE id_responsable = ?))
            OR (den.type_equipe = 'hors_up' AND den.id_equipe IN (SELECT id_up FROM equipe_hors_up WHERE id_responsable = ?))
       )
     ORDER BY d.date_reception DESC`,
    [idResponsable, idResponsable, idResponsable, idResponsable]
  )
  return rows
}
