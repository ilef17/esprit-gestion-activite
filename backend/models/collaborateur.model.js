...
// responsable(s) de ces sous-équipes, et préférences de compte.
export async function getMonProfil(id) {
  const [rows] = await pool.query(
    `SELECT
      ...
      (SELECT GROUP_CONCAT(DISTINCT r.nom SEPARATOR ', ')
        ...
        WHERE cs.id_collaborateur = c.id_collaborateur) AS responsables
    FROM collaborateur c
  ...
  return {
    ...
    responsables: row.responsables || null,
