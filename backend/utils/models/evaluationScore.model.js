...
// Ne compte que les tâches déjà ASSIGNÉES (id_collaborateur renseigné) : les tâches
// encore dans le pool commun (voir "Tâches disponibles à choisir") ne doivent pas
// diluer la moyenne d'équipe utilisée pour comparer la charge de chaque membre.
async function getAvgTaskCount(teamId, type, nbMembres) {
  if (type === 'hors_up') return 0
  const [[{ totalTaches }]] = await pool.query(
    'SELECT COUNT(*) AS totalTaches FROM tache WHERE id_sous_equipe = ? AND id_collaborateur IS NOT NULL',
    [teamId]
  )
...
async function computeSuggestedRatios({ idCollaborateur, teamId, type, avgTaskCount, activiteEcoleRatio }) {
  ...
  // Volume de travail pris en charge : le nombre de tâches que le collaborateur a en
  // tout (à faire, en cours, faites, à refaire — peu importe le statut), comparé à la
  // moyenne de son équipe. On compte bien TOUTES les tâches assignées, pas seulement
  // les terminées : un collaborateur qui a pris en charge beaucoup de tâches doit être
  // reconnu pour cet engagement même avant qu'elles soient toutes closes.
  const nbTachesAssignees = taches.length
  const volumeRatio = avgTaskCount > 0 ? Math.min(nbTachesAssignees / avgTaskCount, 1) : (nbTachesAssignees > 0 ? 1 : 0)
...
// Recalcul automatique et silencieux du score d'UNE équipe, pour la période active —
// appelé en effet de bord dès qu'une tâche change (créée, assignée, statut modifié,
// supprimée) pour que le score affiché reste toujours à jour SANS que l'admin ait à
// cliquer sur "Recalculer maintenant". Les erreurs sont interceptées ici : un souci de
// recalcul ne doit jamais faire échouer l'action tâche qui l'a déclenché — le bouton
// manuel de la page Évaluation reste de toute façon disponible en filet de sécurité.
export async function recalculerScoreEquipeAuto(teamId, type = 'up') {
  if (!teamId) return
  try {
    await calculerScoresEquipe(teamId, {}, type)
  } catch (err) {
    console.error(`Erreur recalcul auto du score (équipe ${type} #${teamId}):`, err.message)
  }
}
