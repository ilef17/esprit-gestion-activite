...
import { calculerScoresPourTous } from './models/evaluationScore.model.js'
...
// Recalcul automatique de TOUS les scores d'évaluation, chaque nuit à 03:00 — les
// changements de tâches (création, assignation, statut, suppression) déclenchent déjà
// un recalcul immédiat de l'équipe concernée (voir recalculerScoreEquipeAuto dans
// taches.controller.js), mais ce filet de sécurité couvre aussi les sources qui ne
// passent pas par une tâche (encadrements, expertises, activités académiques ajoutés
// ailleurs) pour que les scores ne restent jamais silencieusement obsolètes.
cron.schedule('0 3 * * *', async () => {
  try {
    const resultats = await calculerScoresPourTous()
    const erreurs = resultats.filter((r) => r.error)
    if (erreurs.length > 0) console.error('Recalcul nocturne des scores — erreurs:', erreurs)
    else console.log(`Recalcul nocturne des scores effectué (${resultats.length} équipe(s))`)
  } catch (err) {
    console.error('Erreur planificateur de recalcul des scores:', err)
  }
})