import { getScores, calculerScoresEquipe, calculerScoresPourTous, getGrilleNotes } from '../models/evaluationScore.model.js'
...
export async function calculerTout(req, res) {
  try {
    const { annee_universitaire, semestre } = req.body || {}
    const resultats = await calculerScoresPourTous({ annee_universitaire, semestre })
    res.json({ resultats })
  } catch (err) {
    console.error(err)
    res.status(500).json({ message: 'Erreur serveur' })
