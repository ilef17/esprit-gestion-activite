...
import { getMesScores, getHistoriqueScores } from '../models/evaluationScore.model.js'
...
export async function getMe(req, res) {
  try {
    ...
    const historique = await getHistoriqueScores(req.user.id)
    res.json({ ...profil, mes_scores: mesScores, historique_scores: historique })
  } catch (err) {
    console.error(err)
    res.status(500).json({ message: 'Erreur serveur' })
  }
