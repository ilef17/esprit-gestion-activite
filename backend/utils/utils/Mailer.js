...
// Échappe le HTML des valeurs insérées dans les gabarits (ex. titres de tâches
// saisis librement par un responsable), pour éviter toute injection dans l'e-mail.
...
function buildTachesDisponiblesHtml({ nom, equipeNom, nbTaches, titres }) {
  const listeTitresHtml = (titres && titres.length > 1)
    ? `<ul style="margin:0 0 22px; padding-left:18px; font-size:13px; color:${COLORS.text}; line-height:1.7;">
        ${titres.slice(0, 15).map((t) => `<li>${escapeHtml(t)}</li>`).join('')}
        ${titres.length > 15 ? `<li>… et ${titres.length - 15} autre(s)</li>` : ''}
      </ul>`
    : ''
  return `
<!DOCTYPE html>
...
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>Nouvelles tâches disponibles</title>
</head>
<body style="margin:0; padding:0; background:${COLORS.bg}; font-family:Arial, Helvetica, sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${COLORS.bg}; padding:32px 16px;">
    <tr>
      <td align="center">
        <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="max-width:480px; width:100%; background:${COLORS.card}; border-radius:16px; overflow:hidden; box-shadow:0 8px 30px rgba(20,20,40,0.08);">
          ...
          <tr>
            <td style="padding:32px;">
              <h1 style="margin:0 0 14px; font-size:18px; color:${COLORS.text};">Nouvelles tâches à choisir</h1>
              <p style="margin:0 0 22px; font-size:13.5px; color:${COLORS.textMuted}; line-height:1.6;">
                Bonjour <strong style="color:${COLORS.text};">${nom}</strong>, le responsable de <strong style="color:${COLORS.text};">${equipeNom}</strong> vient de publier
                ${nbTaches > 1 ? `${nbTaches} nouvelles tâches` : 'une nouvelle tâche'} disponibles pour votre équipe.
              </p>

              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:22px;">
                <tr>
                  <td style="padding:14px; background:${COLORS.redTint}; border-radius:10px;">
                    <div style="font-size:14px; font-weight:700; color:${COLORS.text};">${nbTaches} tâche${nbTaches > 1 ? 's' : ''} à choisir</div>
                  </td>
                </tr>
              </table>
              ...
              ${listeTitresHtml}
              <p style="margin:0; font-size:12.5px; color:${COLORS.textFaint}; line-height:1.6;">
                Connectez-vous à votre espace collaborateur, onglet "Mes tâches", pour choisir celle(s) qui vous conviennent — dans la limite du nombre de tâches que vous pouvez avoir en cours en même temps.
              </p>
            </td>
          </tr>
...
function buildTachesDisponiblesText({ nom, equipeNom, nbTaches, titres }) {
  const listeTitresText = (titres && titres.length > 1)
    ? `\n${titres.slice(0, 15).map((t) => `  • ${t}`).join('\n')}${titres.length > 15 ? `\n  … et ${titres.length - 15} autre(s)` : ''}\n\n`
    : '\n\n'
  return (
    `Bonjour ${nom},\n\n` +
    `Le responsable de ${equipeNom} vient de publier ${nbTaches > 1 ? `${nbTaches} nouvelles tâches` : 'une nouvelle tâche'} disponibles pour votre équipe.` +
    listeTitresText +
    `Connectez-vous à votre espace collaborateur, onglet "Mes tâches", pour choisir celle(s) qui vous conviennent — dans la limite du nombre de tâches que vous pouvez avoir en cours en même temps.\n\n` +
    `— L'équipe ESPRIT · Gestion des Activités`
  )
...
// Notifie par e-mail un membre d'équipe qu'une nouvelle liste de tâches non assignées
// vient d'être publiée par le responsable et peut être choisie librement (voir
// taches.controller.js -> notifierEquipeNouvellesTaches). Fire-and-forget, comme les
...
export async function sendTachesDisponiblesEmail({ to, nom, equipeNom, nbTaches, titres }) {
  ...
  const from = process.env.SMTP_FROM || '"ESPRIT · Gestion des Activités" <no-reply@esprit.local>'
  const payload = { nom, equipeNom, nbTaches, titres }

  await getTransporter().sendMail({
    from,
    to,
    subject: `${nbTaches} tâche${nbTaches > 1 ? 's' : ''} disponible${nbTaches > 1 ? 's' : ''} — ${equipeNom}`,
    text: buildTachesDisponiblesText(payload),
    html: buildTachesDisponiblesHtml(payload),
    attachments: [ESPRIT_LOGO_ATTACHMENT],
  })
}
