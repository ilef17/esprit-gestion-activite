...
CREATE TABLE IF NOT EXISTS `demande_hors_equipe` (
  ...
  `statut` enum('attente','envoye','validee','refusee') NOT NULL DEFAULT 'attente',
  `date_validation` datetime DEFAULT NULL,
...
-- --------------------------------------------------------
-- campagne_voeux_pedagogiques / reponse_voeu_pedagogique  (NOUVELLES TABLES —
-- questionnaire de vœux pédagogiques : 8 questions fixes, listes de choix de
-- modules gérées par l'admin par campagne, réponses des collaborateurs)
-- --------------------------------------------------------
CREATE TABLE IF NOT EXISTS `campagne_voeux_pedagogiques` (
  ...
  `choix_modules` JSON DEFAULT NULL,
  `choix_modules_alternance` JSON DEFAULT NULL,
  `choix_modules_international` JSON DEFAULT NULL,
  `choix_niveau` varchar(50) DEFAULT NULL,
  `choix_niveau_alternance` varchar(50) DEFAULT NULL,
  `choix_niveau_international` varchar(50) DEFAULT NULL,
  `date_creation` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `reponse_voeu_pedagogique` (
  ...
  `id_collaborateur` int(11) NOT NULL,
  `modules_souhaites` JSON DEFAULT NULL,
  `alternance` enum('oui','non') DEFAULT NULL,
  `modules_alternance` JSON DEFAULT NULL,
  `international` enum('oui','non') DEFAULT NULL,
  `modules_international` JSON DEFAULT NULL,
  `heures_sup` enum('oui','non') DEFAULT NULL,
  `nb_heures_sup` int(11) DEFAULT NULL,
  ...
  KEY `id_collaborateur` (`id_collaborateur`),
  CONSTRAINT `reponse_voeu_ped_ibfk_1` FOREIGN KEY (`id_campagne`) REFERENCES `campagne_voeux_pedagogiques` (`id_campagne`) ON DELETE CASCADE,
  CONSTRAINT `reponse_voeu_ped_ibfk_2` FOREIGN KEY (`id_collaborateur`) REFERENCES `collaborateur` (`id_collaborateur`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Un collaborateur peut être affecté à plusieurs modules (normal / alternance /
-- international) pour une même réponse : une ligne par affectation.
CREATE TABLE IF NOT EXISTS `affectation_voeu_pedagogique` (
  ...
  CONSTRAINT `affectation_voeu_ped_ibfk_1` FOREIGN KEY (`id_reponse`) REFERENCES `reponse_voeu_pedagogique` (`id_reponse`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Migration pour une base déjà existante : voir migration_affectation_voeux_pedagogiques.sql,
-- migration_affectation_type.sql, puis migration_affectations_multiples.sql (ALTER TABLE, à lancer manuellement)
