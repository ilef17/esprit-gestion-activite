...
-- --------------------------------------------------------
-- annee_universitaire_creee (NOUVELLE TABLE)
-- --------------------------------------------------------
-- Historique permanent, en base, de toutes les années universitaires
-- créées/actives. Corrige le bug où une nouvelle année définie comme "période
-- active" disparaissait des sélecteurs Année (Admin / Responsable /
-- Collaborateur) après déconnexion/reconnexion, faute d'être persistée.
-- Même table que celle ajoutée côté application desktop (JavaFX) du collègue
-- (voir ParametreSystemeDAO.ensureAnneesTableExists()), pour que les deux
-- bases restent alignées. Auto-provisionnée aussi au démarrage du serveur
-- (voir ensureAnneeUniversitaireSchema() dans utils/ensureSchema.js) ; voir
-- migration_annee_universitaire_creee.sql pour l'appliquer manuellement /
-- rattraper l'historique sur une base déjà existante.
